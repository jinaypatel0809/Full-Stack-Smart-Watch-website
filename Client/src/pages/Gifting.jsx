  ...
  return (
    ...
    >
      <div onClick={() => navigate(`/product/${product._id}`)}
        className="relative bg-gray-50 cursor-pointer overflow-hidden" style={{ height: 250 }}>
        <img src={product.thumb || product.image} alt={product.name}
          className="w-full h-full object-contain p-3 transition-transform duration-300"
          style={{ transform: hovered ? "scale(1.07)" : "scale(1)" }}
          onError={(e) => { e.target.src = "https://placehold.co/300x210?text=Gift"; }} />
        {product.badge && (
          <span className="absolute top-2.5 left-2.5 bg-orange-500 text-white text-[9px] font-black px-2.5 py-0.5 rounded-full tracking-wide">{product.badge}</span>
        )}
        {product.discount && (
          <span className="absolute top-2.5 right-2.5 bg-green-100 text-green-700 text-[9px] font-black px-2 py-0.5 rounded-full">{product.discount}% OFF</span>
        )}
        <div className="absolute inset-0 flex items-center justify-center transition-all duration-200"
          style={{ background: hovered ? "rgba(0,0,0,0.07)" : "transparent" }}>
          {hovered && <span className="bg-white text-gray-900 text-[11px] font-black px-4 py-2 rounded-full shadow-lg">View Details</span>}
        </div>
      </div>
      ...
      <div className="p-3 flex flex-col gap-1.5 flex-1">
        ...
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[15px] font-black text-gray-900">₹{product.price?.toLocaleString()}</span>
          {product.oldPrice && <span className="text-[11px] text-gray-400 line-through">₹{product.oldPrice?.toLocaleString()}</span>}
          {product.discount && <span className="text-[11px] font-bold text-orange-500">{product.discount}% off</span>}
        </div>
...
export default function Gifting() {
  const navigate = useNavigate();
  const { products: dbProducts, loading } = useProducts("gifting");
  ...
  const handleAddToCart = (product) => {
    addToCart(
      { id: product._id, name: product.name, shortName: product.shortName, price: product.price,
        oldPrice: product.oldPrice, discount: product.discount, thumb: product.thumb || product.image },
      product.colors?.[0]?.name || "Default", 1, isLoggedIn, () => navigate("/login")
    );
  };
  ...
  return (
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      {/* Hero Banner */}
      <div className="relative overflow-hidden text-center py-12 px-4"
        style={{ background: "linear-gradient(135deg,#111 0%,#1a0d0d 60%,#2d1010 100%)" }}>
        <div className="absolute inset-0 pointer-events-none" style={{
          backgroundImage:
            "radial-gradient(circle at 20% 50%,rgba(249,115,22,.13) 0%,transparent 60%)," +
            "radial-gradient(circle at 80% 20%,rgba(244,63,94,.08) 0%,transparent 50%)",
        }} />
        <p className="text-orange-500 text-[11px] font-black tracking-[4px] uppercase mb-2">Fastrack Collection</p>
        <h1 className="text-white text-4xl font-black tracking-tight">Gifting</h1>
        <p className="text-gray-400 text-sm mt-2">
          {loading ? "Loading collection..." : dbProducts.length === 0
            ? "No gifting products added yet"
            : `${filtered.length} perfect gifts for every occasion`}
        </p>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-5 py-7">
        ...
        {/* Filter Bar */}
        <div className="bg-white rounded-2xl border border-gray-100 px-5 py-4 mb-6 flex flex-wrap gap-3 items-center"
          style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
          <div className="relative flex-1 min-w-[180px]">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-[15px]">🔍</span>
            <input type="text" placeholder="Search gifting products..." value={search}
          ...
          {Object.entries(FILTERS).map(([key, cfg]) => (
            <select key={key}
              ...
              style={{ borderColor: selected[key]?.length ? "#f97316" : "#e5e7eb",
                background: selected[key]?.length ? "#fff7ed" : "#fafaf8",
                color: selected[key]?.length ? "#f97316" : "#374151" }}>
              <option value="">{cfg.label}{selected[key]?.length ? ` (${selected[key].length})` : ""}</option>
              {cfg.options.map((opt) => (
                <option key={opt} value={opt}>{selected[key]?.includes(opt) ? "✓ " : ""}{opt}</option>
              ))}
            </select>
          ))}
            ...
            {Object.entries(selected).flatMap(([key, values]) =>
              (values || []).map((val) => (
                <button key={`${key}-${val}`} onClick={() => toggleFilter(key, val)}
                  ...
                  {val}<span className="text-base leading-none">×</span>
                </button>
              ))
            )}
          </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-20 gap-3 text-gray-400">
            ...
            <span className="text-sm font-medium">Loading gifting products...</span>
          </div>
        )}
        ...
        {!loading && dbProducts.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-lg font-black text-gray-800 mb-2">No gifting products added yet</p>
            <p className="text-sm text-gray-400">Admin dashboard → "gifting" section maa products add karo.</p>
          </div>
        )}
        ...
        {!loading && filtered.length > 0 && (
          <p className="text-[13px] text-gray-400 mb-5">
            Showing <strong className="text-gray-800">{filtered.length}</strong> gifting products
          </p>
        )}
