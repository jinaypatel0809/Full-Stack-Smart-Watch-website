    ...
    >
      ...
      >
        <img
          ...
          onError={(e) => { e.target.src = "https://placehold.co/300x210?text=Watch"; }}
        />
...
export default function Watches() {
  const navigate   = useNavigate();
  const { products: dbProducts, loading } = useProducts("watches");
  ...
  return (
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg,#111 0%,#1f1f1f 60%,#2d1a0e 100%)" }}
      >
        <div
        ...
        <h1 className="text-white text-4xl font-black tracking-tight">Watches</h1>
        <p className="text-gray-400 text-sm mt-2">
          {loading
            ? "Loading collection..."
            : dbProducts.length === 0
            ? "No watches added yet"
            : `${filtered.length} styles for every wrist`}
        </p>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-5 py-7">
        ...
        {loading && (
          <div className="flex items-center justify-center py-20 gap-3 text-gray-400">
            ...
            <span className="text-sm font-medium">Loading watches...</span>
          </div>
        )}

        {/* ── No DB products at all ── */}
        {!loading && dbProducts.length === 0 && (
          <div className="text-center py-24">
            <p className="text-5xl mb-4">⌚</p>
            <p className="text-lg font-black text-gray-800 mb-2">No watches added yet</p>
            <p className="text-sm text-gray-400">
              Admin dashboard → "watches" section maa products add karo, aena pachhi ahiya show thase.
            </p>
          </div>
        )}
        ...
        {!loading && filtered.length > 0 && (
          <p className="text-[13px] text-gray-400 mb-5">
            Showing <strong className="text-gray-800">{filtered.length}</strong> watches
          </p>
        )}
