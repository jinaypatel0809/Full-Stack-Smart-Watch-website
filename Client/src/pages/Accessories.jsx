    ...
    >
      <div onClick={() => navigate(`/product/${product._id}`)}
        ...
        <img src={product.thumb || product.image} alt={product.name}
          ...
          onError={(e) => { e.target.src = "https://placehold.co/300x210?text=Accessory"; }} />
        {product.badge && (
...
export default function Accessories() {
  const navigate = useNavigate();
  const { products: dbProducts, loading } = useProducts("accessories");
  ...
  return (
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      <div className="relative overflow-hidden text-center py-12 px-4"
        style={{ background: "linear-gradient(135deg,#111 0%,#0f1a0f 60%,#112211 100%)" }}>
        <div className="absolute inset-0 pointer-events-none" style={{
          backgroundImage:
            "radial-gradient(circle at 20% 50%,rgba(249,115,22,.13) 0%,transparent 60%)," +
            "radial-gradient(circle at 80% 20%,rgba(34,197,94,.08) 0%,transparent 50%)",
        }} />
        <p className="text-orange-500 text-[11px] font-black tracking-[4px] uppercase mb-2">Fastrack Collection</p>
        <h1 className="text-white text-4xl font-black tracking-tight">Accessories</h1>
        <p className="text-gray-400 text-sm mt-2">
          {loading ? "Loading collection..." : dbProducts.length === 0
            ? "No accessories added yet"
            : `${filtered.length} accessories to complete your look`}
        </p>
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-5 py-7">
        ...
        {loading && (
          <div className="flex items-center justify-center py-20 gap-3 text-gray-400">
            ...
            <span className="text-sm font-medium">Loading accessories...</span>
          </div>
        )}
        ...
        {!loading && dbProducts.length === 0 && (
          <div className="text-center py-24">
            ...
            <p className="text-lg font-black text-gray-800 mb-2">No accessories added yet</p>
            <p className="text-sm text-gray-400">Admin dashboard → "accessories" section maa products add karo.</p>
          </div>
        )}
        ...
        {!loading && filtered.length > 0 && (
          <p className="text-[13px] text-gray-400 mb-5">
            Showing <strong className="text-gray-800">{filtered.length}</strong> accessories
          </p>
        )}
